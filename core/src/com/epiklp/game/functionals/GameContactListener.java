    ...
    public void beginContact(Contact contact) {
        ...
        if (aIsSen && bIsSen && a.getBody().getUserData() instanceof Weapon && b.getBody().getUserData() instanceof Weapon) {
            if (a.getBody().getUserData() instanceof FireBall && b.getBody().getUserData() instanceof TowerFireBall) {
                ...
                WeaponCollision.collisionMissiles(fireBall, towerFireBall);
            } else if (a.getBody().getUserData() instanceof FireBall && b.getBody().getUserData() instanceof SlimeBall) {
                FireBall fireBall = (FireBall) a.getBody().getUserData();
                SlimeBall slimeBall = (SlimeBall) b.getBody().getUserData();
                WeaponCollision.collisionMissiles(fireBall, slimeBall);
            } else if (b.getBody().getUserData() instanceof FireBall && a.getBody().getUserData() instanceof SlimeBall) {
                FireBall fireBall = (FireBall) b.getBody().getUserData();
                SlimeBall slimeBall = (SlimeBall) a.getBody().getUserData();
                WeaponCollision.collisionMissiles(fireBall, slimeBall);
            }
            ...
            if (a.getBody().getUserData() instanceof Hero && Utils.equalsWithNulls(b.getBody().getUserData(), "BOSS_MAP")) {
                GameLevel.BOSS_MAP = true;
            } else if (b.getBody().getUserData() instanceof Hero && Utils.equalsWithNulls(a.getBody().getUserData(), "BOSS_MAP")) {
                GameLevel.BOSS_MAP = true;
            }
        }
